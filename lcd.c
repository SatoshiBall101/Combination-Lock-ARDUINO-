...
#define CTRL_BITS ((1 << PB1)|(1 << PB0))cle
...
void lcd_init(void)
{
    ...
    DDRB |= 0x03;   // PB0 and PB1 are outputs
    DDRD |= 0xF0;   // PD4 through PD7 are outputs
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    PORTD &= 0x0f; // clear upper 4 bits of portd

    PORTD = PORTD | (lcdbits & 0xf0); // copy upper 4 bits into portd

    /* Make E signal (PB1) go to 1 and back to 0 */

    PORTB |= (1 << 1); // pb1 goes to 1
    PORTB |= (1 << 1); // pb1 goes to 1
    PORTB &= ~(1 << 1); // pp1 goes to 0
